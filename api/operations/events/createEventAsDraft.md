---
url: >-
  https://developer-portal.gainsight.com/docs/api/operations/events/createEventAsDraft.md
---
...
```
POST https://api2-eu-west-1.insided.com/events/events/draft
```
...
| `type` | string | No | — |
| `content` | string | Yes | — |
| `startsAt` | string | No | ISO format date-time representing the start date and time of the event |
| `endsAt` | string | No | ISO format date-time representing the end date and time of the event |
| `timezone` | string | No | A valid timezone name from the IANA database. For reference: [list of IANA timezone names](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) |
| `location` | string | No | — |
| `url` | string | No | — |
| `image` | string | No | — |
